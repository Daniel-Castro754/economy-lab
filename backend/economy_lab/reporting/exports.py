...
def simulation_xlsx_bytes(scenario: ScenarioSpec, result: SimulationResult) -> bytes:
    ...
    sheets: list[tuple[str, list[list[Any]]]] = [
        ("Resumo", [["Economy Lab v2.11", result.scenario], ["Modelo", result.model], ["Aviso", result.warning], []] + _kv_rows(summary, "Indicador")),
...
def batch_xlsx_bytes(result: BatchExperimentResponse) -> bytes:
    sheets: list[tuple[str, list[list[Any]]]] = [
        ("Resumo", [["Economy Lab v2.11", "Experimento em lote"], ["Eixo", result.axis], ["Repetições", result.repetitions], ["Execuções", result.total_runs], ["Analytics", result.analytics_engine], ["Aviso", result.warning]]),
...
def calibration_xlsx_bytes(scenario, calibration, fit=None) -> bytes:
    ...
    sheets: list[tuple[str, list[list[Any]]]] = [
        ("Resumo calibração", [
            ["Economy Lab v2.11", "Calibration/Data Layer"],
...
def simple_xlsx_bytes(result: SimpleRunResult) -> bytes:
    ...
    sheets = [
        ("Resumo", [["Economy Lab v2.11", "Simple Macro"], ["Cenário", result.config.scenario_id], ["Anos executados", result.completed_years], ["Aviso", result.warning], [],
